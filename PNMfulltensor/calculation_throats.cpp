...
double *throats_length_x(long num_layer, long num_in_row, double pore_bodies[])
{
    ...
    for (int i = 0; i < num_layer - 1; i++)
    {
        ...
        {
            array_1 = pore_bodies[i * num_in_row + j];
            array_2 = pore_bodies[(i + 1) * num_in_row + j];
        }
...
double *throats_length_y(long num_layer, long num_in_row, double pore_bodies[])
{
    ...
    for (int i = 0; i < num_layer - 1; i++)
    {
        for (int j = 0; j < num_in_row; j++)
        {
            array_1 = pore_bodies[j * num_in_row + i];
            array_2 = pore_bodies[j * num_in_row + i + 1];
        }
        max_array1 = rad_max(array_1);
        max_array2 = rad_max(array_2);
        LT[i] = max_array1 + max_array2;
...
double *throats_length_diagonal(double length_x[], double length_y[])
{
    int len_x;
    int len_y;
    len_x = sizeof(length_x) / sizeof(double);
    len_y = sizeof(length_y) / sizeof(double);
    double *LD = new double[len_x * len_y];
    int index;
